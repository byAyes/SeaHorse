...
export class LocalCollection<T extends { id: string }> {
  ...
  async aggregate(opts: { _avg?: { score?: boolean } }): Promise<{ _avg: { score: number | null } }> {
    if (!opts._avg?.score) return { _avg: { score: null } }
    const map = this.getMap()
    const values = Array.from(map.values())
    const scores = values.map((v) => (v as unknown as Record<string, unknown>).score).filter((s): s is number => typeof s === 'number')
  ...
  private filter(where: WhereClause<T>): T[] {
    ...
    for (const key of keys) {
      ...
      // In operator
      if (isInOp(condition)) {
        results = results.filter((item) => condition.in!.includes(item[key] as T[keyof T] & T[keyof T][]))
        continue
      }
