...
async function cleanupZombieRuns() {
  ...
  try {
    ...
    if (result.count > 0) {
      console.log(
        `[Pipeline] Limpieza de zombies: ${result.count} ejecuciones abandonadas marcadas como error`,
      );
    }
  } catch {
...
async function executePipelineRun(runId: string, profile?: Record<string, unknown>) {
  ...
  try {
    ...
    if (profile && allJobs.length > 0) {
      ...
      addLog(
        runId,
        `  Excelentes (≥80): ${excellent} | Buenos (60-79): ${good} | Potenciales (<60): ${matches.length - excellent - good}`,
      );
      await saveToDb();
    ...
    await prisma.pipelineRun
      .create({
        data: {
          ...
          status: 'running',
          logs: [
            `[${now.toLocaleTimeString()}] Pipeline iniciado...`,
            `[${now.toLocaleTimeString()}] ID de ejecución: ${runId}`,
          ],
          startedAt: now,
        },
      })
