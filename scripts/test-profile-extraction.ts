...
async function main() {
  ...
  console.log(
    `  Skills (${p.skills.length}):         ${p.skills.slice(0, 16).join(', ') || '(none)'}${p.skills.length > 16 ? '...' : ''}`,
  );
  ...
  console.log(
    `  Salary Range:      ${p.salaryRange ? `${p.salaryRange.min || '?'} – ${p.salaryRange.max || '?'} ${p.salaryRange.currency || ''}` : 'N/A'}`,
  );
  console.log(
    `  Languages:         ${p.languages.map((l: { language: string; level?: string }) => `${l.language} (${l.level || '?'})`).join(', ') || '(none)'}`,
  );
