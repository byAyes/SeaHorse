...
async function main() {
  ...
  try {
    ...
    geminiResult = { success: false, error: errorMessage };
    geminiTime = -1;
  ...
  try {
    ...
    const errorMessage = err instanceof Error ? err.message : String(err);
    keywordResult = { success: false, error: errorMessage };
    keywordTime = -1;
    console.log(`  FAILED: ${errorMessage}`);
  }
  ...
  console.log();
  console.log(
    `  Duration:          Gemini ${geminiTime > 0 ? `${geminiTime}ms` : 'N/A'}          Keyword ${keywordTime > 0 ? `${keywordTime}ms` : 'N/A'}`,
  );
  console.log();
  ...
  compareField(
    'Summary (first 80)',
    gP?.summary?.substring(0, 80) ?? '',
    kP?.summary?.substring(0, 80) ?? '',
  );
  console.log();
  ...
  const fieldsToCompare = [
    ...
    [
      'skills (first 5)',
      JSON.stringify(gP?.skills?.slice(0, 5) ?? []),
      JSON.stringify(kP?.skills?.slice(0, 5) ?? []),
    ],
    ...
    [
      'salaryRange',
      JSON.stringify(gP?.salaryRange ?? null),
      JSON.stringify(kP?.salaryRange ?? null),
    ],
  ];
  ...
  if ((gP?.skills?.length ?? 0) > (kP?.skills?.length ?? 0))
    geminiAdvantages.push('Extracts more skills');
  if ((gP?.industries?.length ?? 0) > (kP?.industries?.length ?? 0))
    geminiAdvantages.push('Identifies industries');
  if (gP?.languages?.length && !kP?.languages?.length)
    geminiAdvantages.push('Extracts languages with proficiency levels');

  const keywordAdvantages: string[] = [];
  if ((kP?.skills?.length ?? 0) >= (gP?.skills?.length ?? 0) && (kP?.skills?.length ?? 0) > 0)
    keywordAdvantages.push('Matches or beats AI on skill count (no API needed)');
  if (JSON.stringify(kP?.salaryRange) === JSON.stringify(gP?.salaryRange))
    keywordAdvantages.push('Matches AI on salary parsing');
  if (JSON.stringify(kP?.jobTitles) === JSON.stringify(gP?.jobTitles))
    keywordAdvantages.push('Matches AI on job title extraction');
  ...
  console.log(
    '  Keyword speed:    '.padEnd(30) + `${keywordTime > 0 ? `${keywordTime}ms` : 'N/A'}`,
  );
